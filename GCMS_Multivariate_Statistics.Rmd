---
title: "Chemical characterization of the metabolites present in different structures of B. grandiflora by GC-MS metabolomics - Multivariate statistics"
author: "Jefferson Pastuña"
date: "2025-07-01"
output:
  github_document:
    toc: true
    toc_depth: 3
---

```{r setup, include=FALSE}

knitr::opts_chunk$set(echo = TRUE)
usethis::git_vaccinate()

```

# Introduction

This R Script aims to record the procedure for the statistical analysis of the metabolomic profile of different parts (root, bark y leaf) of the *B. grandiflora* plant.      

# Before to start

The “*notame*” package accepts a feature table that ...

# Notame workflow

The “*notame*” package and other dependency packages were installed as a first step for the analysis.

```{r echo=TRUE, message=FALSE}

 #Notame package installationn
if (!requireNamespace("BiocManager", quietly = TRUE))
   install.packages("BiocManager")

BiocManager::install("notame")

#NotameViz package installation
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
BiocManager::install("notameViz")

#pcaMethods package installation
if (!require("BiocManager", quietly = TRUE))
    install.packages("BiocManager")

BiocManager::install("pcaMethods")
```

Then, the libraries were loaded.
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
# Notame library call
library(notame)
library(notameViz)
library(pcaMethods)

# Library to use "mutate" function
library(tidyverse)

```

Then, a primary path and a log system were added to have a record of each process executed.

```{r echo=TRUE, message=TRUE, warning=FALSE, error=FALSE}

# Main path
ppath <- "../B_grandiflora_metabolomics/"
# Log system
init_log(log_file = paste0(ppath, "Result/notame/GCMS_log.txt"))

```

Next, the MZmine feature list table in “*notame*” format was loaded.
 
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

data <- import_from_excel(file = "Data/notame/GCMS_B_grandiflora_feat_list_2notame.xlsx",
                        sheet = 5, corner_row = 6, corner_column = "P",
                        split_by = c("Column", "Ion Mode"))
data <- fix_object(data)

```

## Preprocessing
The first step of the preprocessing is to change the features with a value equal to 0 to NA.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
# Data extraction
mode <- data
# Change 0 value to NA
mode <- mark_nas(mode, value = 0)

```
Avoid features with a low detection rate in the quality control (QC) sample and the sample groups.

Then, features with low detection rates are flagged and removed.

The “*notame*” package uses two criteria to flag these features: the feature's presence in a percentage of QC injections and the feature's presence in a percentage within a sample group or class. Also, we improve the data acquisition using QCs pooled from each experimental group of samples separately insted of QCs pooled from all samples; we considered features as "Low_qc_detection" if the feature is in at least of 60 % of QCs.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Low detection rate for the all dataset
low_dr <- flag_detection(mode, group = 'Group', qc_limit = 0, group_limit = 3/5)
# Extract clean data
low_dr <- drop_flagged(low_dr)
# Adding the "Low_qc_detection" flag
low_detect1 <-  data.frame(rowData(low_dr))
low_detect1 <- low_detect1 %>% 
   mutate(Flag = case_when(`Detection_rate_Group_Leaves.pool` < 0.6 &
                            `Detection_rate_Group_Roots.pool` < 0.6 &
                            `Detection_rate_Group_Stem.bark.pool` < 0.6 &
                            `Detection_rate_QC` < 0.6 ~ "Low_qc_detection"))


rowData(low_dr) <- low_detect1

percentage <- scales::percent(sum(flag(low_dr) == "Low_qc_detection",
                                  na.rm = TRUE)/nrow(low_detect1))
log_text(paste0("\n", percentage, " of features flagged for Low QC detection rate"))
#extract clean data
low_dr <- drop_flagged(low_dr)

```

The following preprocessing step is drift correction, which is applied using smoothed cubic spline regression.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drift correction for the all dataset
mode <- correct_drift(low_dr)
# Flag low quality features for the all dataset
mode <- flag_quality(mode, condition = "RSD_r < 0.3 & D_ratio_r < 0.6")

```

Adding a unique identifier for the drift result that was added in the last step.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Adding a unique identifier for the drift result

names(rowData(mode))[names(rowData(mode)) == "Flag"] <- "Flag_all"

names(rowData(mode))[names(rowData(mode)) == "RSD"] <- "RSD_all"

names(rowData(mode))[names(rowData(mode))  == "D_ratio"] <- "D_ratio_all"

names(rowData(mode))[names(rowData(mode)) == "RSD_r"] <- "RSD_r_all"

names(rowData(mode))[names(rowData(mode)) == "D_ratio_r"] <- "D_ratio_r_all"

names(rowData(mode))[names(rowData(mode)) == "DC_note"] <- "DC_note_all"

# Adding new empty Flag column
lista1 <- data.frame(rowData(mode)) 
lista1$Flag <- NA
rowData(mode) <- lista1

```

Then, we can visualize the data after drift correction.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
all_corr_bp <- plot_sample_boxplots(mode,
                                    order_by = "Group",
                                    fill_by = "Group",
                                    all_features = TRUE)

# PCA
all_corr_pca <- plot_pca(mode,
                     center = TRUE,
                     shape = "Group",
                     color = "Group",
                     all_features = TRUE)
#call library

library(patchwork)

# Plot
all_corr_pca 
all_corr_bp

```

Extracting the roots, stem bark, and leaves dataset to improve data preprocessing separately.

Extracting the roots dataset.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QC
roots <- drop_qcs( mode)
# Removal of Leaves group from the dataset
roots <- roots[, roots$Plant_part != "Leaves"]
colData(roots) <- droplevels(colData(roots))
# Removal of Stem bark group from the dataset
roots <- roots[, roots$Plant_part != "Stem bark"]
colData(roots) <- droplevels(colData(roots))
# Using a pool of Roots samples as QC
levels(colData(roots)$Group)[levels(colData(roots)$Group)=="Roots pool"] <- "QC"
colData(roots)$QC <- colData(roots)$Group
levels(colData(roots)$QC)[levels(colData(roots)$QC)=="Roots"] <- "Sample"



```

Extracting the stem bark dataset.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QC
bark <- drop_qcs( mode)
# Removal of Leaves group from the dataset
bark <- bark[, bark$Plant_part != "Leaves"]
colData(bark) <- droplevels(colData(bark))
# Removal of Roots group from the dataset
bark <- bark[, bark$Plant_part != "Roots"]
colData(bark) <- droplevels(colData(bark))
# Using a pool of Stem bark samples as QC
levels(colData(bark)$Group)[levels(colData(bark)$Group)=="Stem bark pool"] <- "QC"
colData(bark)$QC <- colData(bark)$Group
levels(colData(bark)$QC)[levels(colData(bark)$QC)=="Stem bark"] <- "Sample"

```

Extracting the leaves dataset.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QC
leaves <- drop_qcs( mode)
# Removal of Stem bark group from the dataset
leaves <- leaves[, leaves$Plant_part != "Stem bark"]
colData(leaves) <- droplevels(colData(leaves))
# Removal of Roots group from the dataset
leaves <- leaves[, leaves$Plant_part != "Roots"]
colData(leaves) <- droplevels(colData(leaves))
# Using a pool of leaves samples as QC
levels(colData(leaves)$Group)[levels(colData(leaves)$Group)=="Leaves pool"] <- "QC"
colData(leaves)$QC <- colData(leaves)$Group
levels(colData(leaves)$QC)[levels(colData(leaves)$QC)=="Leaves"] <- "Sample"

```

Extracting the QC dataset.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Extracting the QC dataset.
qc_sample <- mode[, mode$Plant_part == "QC"]

```

The following preprocessing step is drift correction, which is applied using smoothed cubic spline regression.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drift correction for the roots dataset
corrected_r <- correct_drift(roots)
# Drift correction for the stem bark dataset
corrected_b <- correct_drift(bark)
# Drift correction for the leaves dataset
corrected_l <- correct_drift(leaves)
# Flag low quality features for the roots dataset
corrected_r <- flag_quality(corrected_r, condition = "RSD_r < 0.3 & D_ratio_r < 0.6")
# Flag low quality features for the stem bark dataset
corrected_b <- flag_quality(corrected_b, condition = "RSD_r < 0.3 & D_ratio_r < 0.6")
# Flag low quality features for the leaves dataset
corrected_l <- flag_quality(corrected_l, condition = "RSD_r < 0.3 & D_ratio_r < 0.6")

```

Adding a unique identifier for the drift result that was added in the last step for each dataset (roots, stem bark, and leaves).

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Adding a unique identifier for the drift result of the roots dataset
names(rowData(corrected_r))[names(rowData(corrected_r)) == "Flag"] <- "Flag_root"
names(rowData(corrected_r))[names(rowData(corrected_r))  == "RSD"] <- "RSD_root"
names(rowData(corrected_r))[names(rowData(corrected_r))  == "D_ratio"] <- "D_ratio_root"
names(rowData(corrected_r))[names(rowData(corrected_r))  == "RSD_r"] <- "RSD_r_root"
names(rowData(corrected_r))[names(rowData(corrected_r))  == "D_ratio_r"] <- "D_ratio_r_root"
names(rowData(corrected_r))[names(rowData(corrected_r))  == "DC_note"] <- "DC_note_root"
# Adding new empty Flag column

lista2 <- data.frame(rowData(corrected_r)) 
lista2$Flag <- NA
rowData(corrected_r) <- lista2

# Adding a unique identifier for the drift result of the stem bark dataset
names(rowData(corrected_b))[names(rowData(corrected_b)) == "Flag"] <- "Flag_bark"
names(rowData(corrected_b))[names(rowData(corrected_b)) == "RSD"] <- "RSD_bark"
names(rowData(corrected_b))[names(rowData(corrected_b)) == "D_ratio"] <- "D_ratio_bark"
names(rowData(corrected_b))[names(rowData(corrected_b)) == "RSD_r"] <- "RSD_r_bark"
names(rowData(corrected_b))[names(rowData(corrected_b)) == "D_ratio_r"] <- "D_ratio_r_bark"
names(rowData(corrected_b))[names(rowData(corrected_b)) == "DC_note"] <- "DC_note_bark"
# Adding new empty Flag column
lista3 <- data.frame(rowData(corrected_b)) 
lista3$Flag <- NA
rowData(corrected_b) <- lista3

# Adding a unique identifier for the drift result of the leaves dataset
names(rowData(corrected_l))[names(rowData(corrected_l)) == "Flag"] <- "Flag_leave"
names(rowData(corrected_l))[names(rowData(corrected_l)) == "RSD"] <- "RSD_leave"
names(rowData(corrected_l))[names(rowData(corrected_l)) == "D_ratio"] <- "D_ratio_leave"
names(rowData(corrected_l))[names(rowData(corrected_l)) == "RSD_r"] <- "RSD_r_leave"
names(rowData(corrected_l))[names(rowData(corrected_l)) == "D_ratio_r"] <- "D_ratio_r_leave"
names(rowData(corrected_l))[names(rowData(corrected_l)) == "DC_note"] <- "DC_note_leave"
# Adding new empty Flag column
lista4 <- data.frame(rowData(corrected_l)) 
lista4$Flag <- NA
rowData(corrected_l) <- lista4
```

Merge the result of drift correction. 

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
merged <- merge_notame_sets(corrected_b, corrected_l, corrected_r, qc_sample, merge = 'samples')

merged_test <- merge_metabosets(corrected_r, corrected_b, corrected_l, qc_sample,
                                merge = "samples")


```

Then, we can visualize the data after drift correction.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
merged_bp <- plot_sample_boxplots(merged,
                                  order_by = "Group",
                                  fill_by = "Group")
# PCA
merged_pca <- plot_pca(merged,
                       center = TRUE,
                       shape = "Group", 
                       color = "Group")
# Plot
merged_pca + merged_bp

```

Adding the "Low_quality" flag for features that were flagged as low quality in all datasets (roots, stem bark, and leaves).

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Adding the "Low_quality" flag
merged1 <- data.frame(rowData(merged))
merged1 <- merged1 %>%
  mutate(Flag = case_when(Flag_all == "Low_quality" &
                            Flag_root == "Low_quality" &
                            Flag_bark == "Low_quality" &
                            Flag_leave == "Low_quality" ~ "Low_quality"))
rowData(merged) <- merged1

# Show the percentage of flagged features as "Low_quality"
percentage <- scales::percent(sum(flag(merged) == "Low_quality",
                                  na.rm = TRUE)/nrow(merged1))
log_text(paste0("\n", percentage, " of features flagged for low quality"))

```

Then, we can visualize the data after falg the low quality features.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
best_bp <- plot_sample_boxplots(merged,
                                  order_by = "Group",
                                  fill_by = "Group")
# PCA
best_pca <- plot_pca(merged,
                       center = TRUE,
                       shape = "Group",
                       color = "Group")
# Plot
best_pca + best_bp

```

The next step is feature clustering. This step helps us reduce the number of features of the same molecule that were split due to a 70 eV electron ionization (EI) environment.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
  
rt_limits <- c(3, 58)

clustered <- cluster_features(merged, rt_window = 0.05/3, corr_thresh = 0.95,
                              d_thresh = 0.80, mz_col = "mass", rt_col = "RT")

compressed <- compress_clusters(clustered)

```

We can inspect the data using the PCA plot after the clustering algorithm execution.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
clust_bp <- plot_sample_boxplots(compressed,
                                 order_by = "Group",
                                 fill_by = "Group")
# PCA
clust_pca <- plot_pca(compressed,
                      center = TRUE,
                      shape = "Group",
                      color = "Group")
# Plot
clust_pca + clust_bp

```

## Processing

We used probabilistic quotient normalization (PQN) with missing value imputation and scaled using the autoscaling method for downstream statistical analysis.

The code below imputes the data using a chromatographic noise value.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Impute missing values using noise threshold
imputed <- impute_simple(compressed, value = 50, na_limit = 0)

```

We can inspect the data with the PCA plot after data imputation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
imp_bp <- plot_sample_boxplots(imputed,
                               order_by = "Group",
                               fill_by = "Group")
# PCA
imp_pca <- plot_pca(imputed,
                    center = TRUE,
                    shape = "Group",
                    color = "Group")
# Plot
imp_pca + imp_bp

```

After data imputation, the data were normalized by PQN.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Probabilistic quotient normalization
pqn_set <- pqn_normalization(imputed,
                             ref = c("qc", "all"),
                             method = c("median", "mean"),
                             all_features = FALSE)

```

We can inspect the data with the PCA plot after data normalization.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Boxplot
pqn_bp <- plot_sample_boxplots(pqn_set,
                               order_by = "Group",
                               fill_by = "Species")
# PCA
pqn_pca <- plot_pca(pqn_set,
                    center = TRUE,
                    shape = "Species",
                    color = "Species")
# Plot
pqn_pca + pqn_bp

```

Finally, the data is ready to be exported and proceed with the statistical analysis.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

save(compressed, file = paste0(ppath, "Result/notame_Result/HS_GCMS/Notame_HS_GC-MS_out.RData")) 

```

# Principal component analysis (PCA)

Extracting the data and metadata of "*notame*" MetaboSet for principal component analysis (PCA) plot.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Extract clean data
pqn_noflag <- drop_flagged(pqn_set)
# Extracting feature height table
peak_height <- assay(pqn_noflag)
# Extracting phenotypic data
pheno_data <- data.frame(pqn_noflag@colData@listData)

```

Transposing the feature table and preparing the PCA data.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Transposing feature height table
transp_table  <- t(peak_height)
# Centering and Scaling features
ei_pca <- prcomp(transp_table, center = TRUE, scale. = TRUE)

```

## Score PCA

Plotting PCA results.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide', fig.width = 7, fig.height = 4}

# Library to left_join use
library(dplyr)
# PCA scores
scores <- ei_pca$x %>%                   # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
  left_join(pheno_data)                  # Adding metadata
# PCA plot
pca_plot <- ggplot(scores,
       aes(PC1, PC2, shape = Group, color = Group)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (19.12 %)"),
         y=guide_axis(title = "PC2 (10.46 %)")) +
  labs(shape = 'Morphological structures', color= 'Morphological structures') +
  theme_classic() +
  theme(legend.text = element_text(face="italic")) +
  theme(legend.position = c(0.83, 0.22),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(fill= "transparent")) +
  geom_vline(xintercept = 0, linetype = "longdash", colour="gray") +
  geom_hline(yintercept = 0, linetype = "longdash", colour="gray")
pca_plot

```

## Loading PCA

Plotting a loading PCA result.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

loadings <- ei_pca$rotation %>%           # Extract loadings
  data.frame(Feature_ID = rownames(.))    # New column with feat name

```

Creating an artificial table with feature names and a column with compound names (identified metabolites).

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide', fig.width = 14, fig.height = 7}

# Extracting feature identified
metab_data <- pqn_noflag[!is.na(pqn_noflag@elementMetadata@listData$Metabolite),]
# Extracting metabolite table
meta_table <- data.frame(metab_data@elementMetadata@listData)
# Creating a new small table of the annotated compounds
ei_compouds <- left_join(meta_table, loadings)

# Plotting results
load_pca <- ggplot(loadings, aes(PC1, PC2)) + 
  geom_point(alpha = 0.3, size = 2) +
  theme_classic() + 
  geom_point(data = ei_compouds,
             aes(shape = meta_table$IL,
                 color = meta_table$IL),
                 size = 1) +
  labs(shape = 'Identification level',
       color = 'Identification level') +
  scale_color_manual(values = c("yellow3", "green")) +
  scale_shape_manual(values = c(17, 19)) +
  ggrepel::geom_label_repel(data = ei_compouds,
                            aes(label = meta_table$Metabolite),
                            box.padding = 0.37,
                            label.padding = 0.22,
                            label.r = 0.30,
                            cex = 2.5,
                            max.overlaps = 50,
                            min.segment.length = 0) +
  guides(x=guide_axis(title = "PC1 (19.12 %)"),
         y=guide_axis(title = "PC2 (10.46 %)")) +
  theme(legend.position = c(0.070, 0.090),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(fill= "transparent")) +
  geom_vline(xintercept = 0, linetype = "longdash", colour="gray") +
  geom_hline(yintercept = 0, linetype = "longdash", colour="gray")
  #ggsci::scale_color_aaas()
load_pca
# Save plot
#ggsave('Result/notame_Result/HS_GCMS/load_pca.pdf', width = 14, height = 7, device='pdf', dpi="print")

```

# Heatmap with HCA

Only the annotated features were used in the heatmap and the hierarchical cluster analysis (HCA). The ComplexHeatmap package will be used for the heatmap and the hierarchical cluster analysis (HCA).

ComplexHeatmap package and dependency installation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# ComplexHeatmap package installation
#if (!requireNamespace("BiocManager", quietly=TRUE))
#    install.packages("BiocManager")
# BiocManager::install("ComplexHeatmap")
library(ComplexHeatmap)

# ColorRamp2 package installation
#if (!requireNamespace("devtools", quietly = TRUE)) {
 # install.packages("devtools")
#}
#devtools::install_github("jokergoo/colorRamp2")
library(colorRamp2)

# Cowplot package installation
#install.packages("cowplot")
library(cowplot)

# mdatools package installation
#install_github('svkucheryavski/mdatools')
library(mdatools)

# ClassyFire package installation
#remotes::install_github('aberHRML/classyfireR')

library(classyfireR)

```

The metabolites were classified using the [ClassyFireR](https://doi.org/10.1186/s13321-016-0174-y) to add these metabolite classifications to the heatmap plot.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# InChI key of the metabolites you want to classify
InChI_Keys <- c('2-Heptanone' = "DTOSIQBPPRVQHS-PDBXOOCHSA-N")
# Get classification
Classification_List <- purrr::map(InChI_Keys, get_classification)
Classification_List


```

Extracting and loading of identified metabolites' abundance, and data scaling by the autoscaling method.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QC 
hm_no_qc <- drop_qcs(pqn_noflag)
# Scaling by autoscaling method
hm_scl <- scale(t(assay(hm_no_qc)), center = TRUE, scale = TRUE)
hm_scl <- t(hm_scl)
# Adding autoscaling data to notame MetaboSet
hm_scl_set <- hm_no_qc
assay(hm_scl_set) <- hm_scl
# Extracting identified metabolite data
raw_hm <- hm_scl_set[!is.na(hm_scl_set@elementMetadata@listData$Metabolite),]
# Extracting feature height table
hm_height <- assay(raw_hm)
# Extracting sample information
hm_pdata <- raw_hm@colData@listData
# Extracting feature information
hm_fdata <- raw_hm@elementMetadata@listData

```

Row and top heatmap annotation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, fig.width = 10, fig.height = 9}

set.seed(1540)
# Adding row and column names
hm_scl <- hm_height
rownames(hm_scl) <- hm_fdata$Metabolite
colnames(hm_scl) <- hm_pdata$Group
# Metabolite class color
cols_metclass <- c("Benzenoids" = "#800000FF",
                   "Hydrocarbons" = "#FFA319FF",
                   "Lipids and lipid-like molecules" = "#8A9045FF",
                   "Organic oxygen" = "#8DD3C7",
                   "Lipids and lipi" = "#8A9045FF",
                   "Phenylpropanoid" = "#BEBADA",
                   "Organoheterocyc" = "#FFFFB3",
                   'Fatty aldehyd'="#FB8072",
                   'Organic acids a'="#80B1D3",
                   'Organic 1,3-dip'='#FDB462')
# Add row anotation to HeatMap
hm_row_ann <- rowAnnotation(`Superclass` = hm_fdata$Classyfire_Superclass,
                            col = list(`Superclass` = cols_metclass),
                            show_annotation_name = T,
                            show_legend = F)
# Species color
cols_plant_part <- c("Leaves" = '#E76BF3',
                  "Stem bark" = "#F8766D",
                  "Roots" = "#7CAE00")

# Add top anotation to Heatmap
top_info_ann <- HeatmapAnnotation(`Group` = hm_pdata$Group,
                                  col = list(`Group` = cols_Plant_part),
                                  show_annotation_name = T,
                                  show_legend = F,
                                  border = T)
# Color scale
mycol <- colorRamp2(c(-4, 0, 4), c("blue", "white", "red"))
# Heatmap matrix plotting
hm_plot <- Heatmap(hm_scl,
                   col = mycol,
                   border_gp = grid::gpar(col = "black", lty = 0.02),
                   rect_gp = grid::gpar(col = "black", lwd = 0.75),
                   clustering_distance_columns = "euclidean",
                   clustering_method_columns = "complete",
                   top_annotation = top_info_ann,
                   column_names_gp = gpar(fontface = "italic", fontsize = 11),
                   row_names_gp = gpar(fontsize = 7), 
                   row_names_max_width = unit(10, "cm"),
                   right_annotation = hm_row_ann,
                   show_heatmap_legend = F,
                   row_km = 3, column_km = 3,
                   row_title = c("a", "b", "c"))
hm_plot

```

Adding legends to the heatmap.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Color scale legend
lgd1 <- Legend(col_fun = mycol,
               title = "Autoscaled abundance",
               direction = "horizontal" )
# Bees species legend
lgd2 <- Legend(labels = gt_render(c("Leaves",
                                    "Stem bark",
                                    "Roots")),
               legend_gp = gpar(fill = cols_plant_part),
               title = "Morphological structures", ncol = 1)
# Metabolite class Legend
lgd3 <- Legend(labels = c(unique(hm_fdata$Classyfire_Superclass)) ,
               legend_gp = gpar(fill = cols_metclass), 
               title = "Metabolite superclass", ncol = 2)

```

Plotting the heatmap.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, fig.width = 10, fig.height = 12}

set.seed(1540)
# Converting to ggplot
gg_heatmap <- grid.grabExpr(draw(hm_plot))
gg_heatmap <- ggpubr::as_ggplot(gg_heatmap)
# Legends
all_legends <- packLegend(lgd1, lgd2, lgd3, direction = "horizontal")
gg_legend <- grid.grabExpr(draw(all_legends))
gg_legend_fn <- ggpubr::as_ggplot(gg_legend)
# Heatmap plot
gcms_hm <- plot_grid(gg_legend_fn,
                     gg_heatmap, ncol = 1,
                     rel_heights = c(0.055, 0.880))
gcms_hm

```

# Antimicrobial activity correlation

Correlation between feature abundance and antioxidant result (TAC) using the PCA and Pearson's correlation approaches.

## Using the PCA

Data and metadata preparation, merging the antimicrobial data with feature abundance.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Removal of qc group from the dataset
antioxidant_data <- drop_qcs(data) 
# Removal of Leaves group from the dataset
antioxidant_data <- antioxidant_data[, antioxidant_data$Group != "Leaves pool"]
colData(antioxidant_data) <- droplevels(colData(antioxidant_data))
# Removal of Stem bark group from the dataset
antioxidant_data <- antioxidant_data[, antioxidant_data$Group != "Stem bark pool"]
colData(antioxidant_data) <- droplevels(colData(antioxidant_data))
# Removal of Roots group from the dataset
antioxidant_data <- antioxidant_data[, antioxidant_data$Group != "Roots pool"]
colData(antioxidant_data) <- droplevels(colData(antioxidant_data))
# Extracting only the antioxidant activity result
antioxidant_data <- antioxidant_data["Antiox_No_IM_80a3", , drop = FALSE] 
# Transposing antioxidant activity data
antioxidant_data <- t(assay(antioxidant_data))

# Drop QCs of metabolomics data
no_qc <- drop_qcs(pqn_noflag)
# Extracting feature height table
corr_peak <- t(assay(no_qc))
corr_peak <- t(assay(pqn_noflag))
# Merge the antioxidant activity result with the feature list table
antioxidant_peak <- cbind(antioxidant_data, corr_peak)
# Extracting phenotypic data of metabolomics data
corr_pdata <- no_qc@colData@listData

```

### Score PCA

PCA scores calculation of the antioxidant and feature abundance merge data.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Centering and Scaling features
corr_pca <- prcomp(antioxidant_peak, center = TRUE, scale. = TRUE)

```

Plotting PCA score.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide', fig.width = 7, fig.height = 4}

# PCA scores
corr_scores <- corr_pca$x %>%            # Get PC coordinates
  data.frame %>%                         # Convert to data frames
  mutate(Sample_ID = rownames(.)) %>%    # Create a new column with the sample names
  left_join(data.frame(corr_pdata))                  # Adding metadata

# PCA plot
corr_pca_plot <- ggplot(corr_scores,
                        aes(PC1, PC2, shape = Group, color = Group)) +
  scale_color_manual(values=c("#F8766D",
                              "#7CAE00",
                              "#E76BF3")) +
  scale_shape_manual(values=c(16, 17, 3)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (20.14 %)"),
         y=guide_axis(title = "PC2 (14.12 %)")) +
  labs(shape = 'Group', color= 'Group') +
  theme_classic() +
  theme(legend.text = element_text(face="italic")) +
  theme(legend.position = c(0.120, 0.200),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(fill= "transparent")) +
  geom_vline(xintercept = 0, linetype = "longdash", colour="gray") +
  geom_hline(yintercept = 0, linetype = "longdash", colour="gray")
corr_pca_plot

```

### Loading PCA

Plotting loading results.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

corr_loadings <- corr_pca$rotation %>%    # Extract loadings
  data.frame(Feature_ID = rownames(.))    # New column with feat name

```

Creating an artificial table with feature names and a column with compound names (identified metabolites).

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide', fig.width = 14, fig.height = 7}

# Extracting the antioxidant metadata
antioxidant_table <- drop_qcs(data)@elementMetadata@listData
antioxidant_table$Flag  <- as.character(antioxidant_table$Flag)
# Merge the antioxidant metadata with the metabolite table
corr_table <- merge(meta_table, antioxidant_table, all = TRUE)
# Creating a new small table of the annotated compounds and antioxidant metadata
corr_compouds <- left_join(corr_table, corr_loadings)
# Plotting results
corr_load_plot <- ggplot(corr_loadings, aes(PC1, PC2)) + 
  geom_point(alpha = 0.3, size = 2) +
  theme_classic() +
  geom_point(data = corr_compouds, aes(shape = corr_table$Antiox,
                                       color = corr_table$Antiox), size = 2.5) +
  labs(shape = 'Legend of features',
       color = 'Legend of features') +
  scale_color_manual(values = c("red",
                                "green",
                                "darkblue")) +
  scale_shape_manual(values = c(15, 17, 19)) +
  ggrepel::geom_label_repel(data = corr_compouds,
                            aes(label = corr_table$Metabolite,
                                fontface = ifelse(corr_table$Antiox == "Antioxidant activity",
                                                  'italic', 'plain')),
                            box.padding = 0.37,
                            label.padding = 0.22,
                            label.r = 0.30,
                            cex = 2.5,
                            max.overlaps = 50,
                            min.segment.length = 0) +
  guides(x=guide_axis(title = "PC1 (22.46 %)"),
         y=guide_axis(title = "PC2 (18.82 %)")) +
  theme(legend.position = c(0.070, 0.107),
        legend.background = element_rect(fill = "white", color = "black")) +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(fill= "transparent")) +
  geom_vline(xintercept = 0, linetype = "longdash", colour="gray") +
  geom_hline(yintercept = 0, linetype = "longdash", colour="gray")
  #ggsci::scale_color_aaas()
corr_load_plot

```

## Using the Pearson correlation

The Pearson correlation between antioxidant activity and feature abundance was performed.

### Normality of the antioxidant data

First, the antioxidant data normality was evaluated using the Shapiro-Wilk test.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Loading the "readxl" package
library(readxl)
# Loading the antioxidant activity data
all_assay <- read_excel("Data/Antimicrobial_of_honey.xlsx", sheet = 1)
# Deleting the group column to perform Shapiro-Wilk test in batch
no_group <- all_bacteria[,-1]
# Perform the Shapiro-Wilk test
shapiro <- do.call(rbind, lapply(no_group,
                                 function(x) shapiro.test(x)[c("statistic", "p.value")]))
shapiro

```

The result showed that only the microorganisms *K. pneumonia* and *P. mirabilis* have a normal distribution (p-value > 0.05). However, when we inspected the data, we found the values in the ATA001 and RIG003 groups in the *E. coli* could be outliers; also, a possible outlier in the RYMG001 group in the *S. aureus* was found. When we delete all these possible outliers, we find that all the microorganisms (not including *S. aureus* at 25 % v/v) have a normal distribution, as seen below.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Delete outlier data in E. coli
no_outliers <- all_bacteria
no_outliers[no_outliers$Group == "ATA001",]$E_coli_75 <- NA
no_outliers[no_outliers$Group == "ATA001",]$E_coli_100 <- NA
no_outliers[no_outliers$Group == "RIG003",]$E_coli_75 <- NA
no_outliers[no_outliers$Group == "RIG003",]$E_coli_100 <- NA
# Delete outlier data in S. aureus
no_outliers[no_outliers$Group == "RYMG001",]$S_aureus_25 <- NA
no_outliers[no_outliers$Group == "RYMG001",]$S_aureus_50 <- NA
no_outliers[no_outliers$Group == "RYMG001",]$S_aureus_75 <- NA
no_outliers[no_outliers$Group == "RYMG001",]$S_aureus_100 <- NA
# Deleting the group column to perform Shapiro-Wilk test in batch
no_outliers <- no_outliers[,-1]
# Perform the Shapiro-Wilk test
shapiro_outl <- do.call(rbind, lapply(no_outliers,
                                 function(x) shapiro.test(x)[c("statistic", "p.value")]))
shapiro_outl

```

The antimicrobial activity result of honey at 75 % v/v was used in the further analysis because this concentration showed activity in most of the tested microorganisms. (*E. coli*, *K. pneumonia*, *P. mirabilis*, and *S. aureus*).

### Metabolomics data transformation

Before the Pearson correlation of feature abundance with the antimicrobial activity (zone inhibition diameter), the previously normalized data by PQN was transformed using the generalised logarithm (glog) method.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# "SummarizedExperiment" package installation
#if (!require("BiocManager", quietly = TRUE))
#    install.packages("BiocManager")
#BiocManager::install("SummarizedExperiment")
library(SummarizedExperiment)
# Convert feature height table to SummarizedExperiment class
pmp_data <- SummarizedExperiment(assays = exprs(pqn_noflag),
                                 colData = pqn_noflag@phenoData@data)
# Package for generalized logarithmic transform
#if (!requireNamespace("BiocManager", quietly = TRUE))
#    install.packages("BiocManager")
#BiocManager::install("pmp")
library(pmp)
# Generalised logarithmic transform
glog_exprs <- glog_transformation(df = pmp_data@assays@data@listData[[1]],
                                  classes = pmp_data$QC,
                                  qc_label = "QC")
# Adding glog transformation to notame MetaboSet
glog_set <- pqn_noflag
exprs(glog_set) <- glog_exprs

```

Preparing the metabolomics data for the antimicrobial correlation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Drop QCs
glog_no_qc <- drop_qcs(glog_set)
# Preparing the data for E. coli correlation (drop outlier data)
glog_E.coli <- glog_no_qc[, glog_no_qc$Group != "ATA001"]
pData(glog_E.coli) <- droplevels(pData(glog_E.coli))
glog_E.coli <- glog_E.coli[, glog_E.coli$Group != "RIG003"]
pData(glog_E.coli) <- droplevels(pData(glog_E.coli))
# Preparing the data for S. aureus (dropn outlier data)
glog_S.aureus <- glog_no_qc[, glog_no_qc$Group != "RYMG001"]
pData(glog_S.aureus) <- droplevels(pData(glog_S.aureus))
# Convert antimicrobial result to numeric
glog_E.coli$E_coli_75 <- as.numeric(glog_E.coli$E_coli_75)
glog_no_qc$K_pneumonia_75 <- as.numeric(glog_no_qc$K_pneumonia_75)
glog_no_qc$P_mirabilis_75 <- as.numeric(glog_no_qc$P_mirabilis_75)
glog_S.aureus$S_aureus_75 <- as.numeric(glog_S.aureus$S_aureus_75)

```

Perform the Pearson correlation analysis.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Feature correlation with the E. coli antimicrobial activity
E.coli_corr <- perform_correlation_tests(glog_E.coli,
                                         x = featureNames(glog_no_qc),
                                         y = c("E_coli_75"),
                                         method = "pearson")
# Feature correlation with the K. pneumonia antimicrobial activity
K.pneumonia_corr <- perform_correlation_tests(glog_no_qc,
                                              x = featureNames(glog_set),
                                              y = c("K_pneumonia_75"),
                                              method = "pearson")
# Feature correlation with the P. mirabilis antimicrobial activity
P.mirabilis_corr <- perform_correlation_tests(glog_no_qc,
                                              x = featureNames(glog_set),
                                              y = c("P_mirabilis_75"),
                                              method = "pearson")
# Feature correlation with the S. aureus antimicrobial activity
S.aureus_corr <- perform_correlation_tests(glog_S.aureus,
                                           x = featureNames(glog_no_qc),
                                           y = c("S_aureus_75"),
                                           method = "pearson")

```

### Correlation of identified metabolites

Create a matrix for the heatplot of Pearson's correlation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Correlation table for the heatmap
hm_corr_table <- data.frame(Feature_ID = E.coli_corr$X,
                            E_coli = E.coli_corr$Correlation_coefficient,
                            K_pneumonia = K.pneumonia_corr$Correlation_coefficient,
                            P_mirabilis = P.mirabilis_corr$Correlation_coefficient,
                            S_aureus = S.aureus_corr$Correlation_coefficient)
# Feature data table
feat_corr_table <- data.frame(Feature_ID = hm_fdata$Feature_ID,
                             Metabolite = hm_fdata$Metabolite)
# Adding the metabolie name to the correlation table
hm_corr_table <- left_join(hm_corr_table, feat_corr_table)
# Extracting data of the identified metabolites
hm_corr_table <- hm_corr_table[!is.na(hm_corr_table$Metabolite),]
# Adding row name
rownames(hm_corr_table) <- hm_corr_table$Metabolite
# Delete extra information
hm_corr_table <- subset(hm_corr_table, select = -c(Feature_ID, Metabolite))
# Converting DataFrame to data matrix
hm_corr_table <- data.matrix(hm_corr_table, rownames.force = NA)
# Adding column name
colnames(hm_corr_table) <- c("E. coli", "K. pneumonia", "P. mirabilis", "S. aureus")

```

Create a matrix showing the statistical significance of the correlation result.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Correlation table for the heatmap
hm_sign_table <- data.frame(Feature_ID = E.coli_corr$X,
                            E_coli = E.coli_corr$Correlation_P,
                            K_pneumonia = K.pneumonia_corr$Correlation_P,
                            P_mirabilis = P.mirabilis_corr$Correlation_P,
                            S_aureus = S.aureus_corr$Correlation_P)
# Adding the metabolie name to the correlation table
hm_sign_table <- left_join(hm_sign_table, feat_corr_table)
# Extracting data of the identified metabolites
hm_sign_table <- hm_sign_table[!is.na(hm_sign_table$Metabolite),]
# Adding row name
rownames(hm_sign_table) <- hm_sign_table$Metabolite
# Delete extra information
hm_sign_table <- subset(hm_sign_table, select = -c(Feature_ID, Metabolite))
# Converting DataFrame to data matrix
hm_sign_table <- data.matrix(hm_sign_table, rownames.force = NA)
# Adding column name
colnames(hm_sign_table) <- c("E. coli", "K. pneumonia", "P. mirabilis", "S. aureus")

```

Plot the correlation heatplot.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, fig.width = 5, fig.height = 9}

# Add top anotation to Heatmap
ann_corr <- HeatmapAnnotation(
  foo = anno_block(gp = gpar(fill = 0, col = "white"),
                   labels = "Pearson correlation",
                   labels_gp = gpar(col = "black", fontsize = 12,
                                    fontface = "bold")))
# Color scale
col_fun <- colorRamp2(c(-1, 0, 1), c("blue", "white", "red"))
# Heatmap plot
hm_corr <- Heatmap(hm_corr_table, col = col_fun, cluster_rows = FALSE,
                   cluster_columns = FALSE, column_names_gp = gpar(fontface = "italic"),
                   right_annotation = hm_row_ann,
                   border_gp = grid::gpar(col = "black", lty = 0.02),
                   rect_gp = grid::gpar(col = "black", lwd = 0.75),
                   top_annotation = ann_corr,
                   cell_fun = function(j, i, x, y, w, h, fill) {
                     if(hm_sign_table[i, j] < 0.05) {
                       grid.text(sprintf("%.1f", hm_corr_table[i, j]), x, y,
                                 gp = gpar(fontsize = 12, fontface = "bold"))
                       } else if(hm_sign_table[i, j] > 0.05) {
                         grid.text(sprintf("%.1f", hm_corr_table[i, j]), x, y,
                                   gp = gpar(fontsize = 12))}},
                   show_heatmap_legend = F)
hm_corr

```

### Correlation of unknown metabolites

Filtering the metabolites with positive and high Pearson's correlation coefficient (r >0.5).

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Library to change character values
library('stringr')
# Correlation table for the heatmap with the best features
best_feat <- data.frame(Feature_ID = E.coli_corr$X,
                        E_coli = E.coli_corr$Correlation_coefficient,
                        E_coli_p = E.coli_corr$Correlation_P,
                        K_pneumonia = K.pneumonia_corr$Correlation_coefficient,
                        K_pneumonia_p = K.pneumonia_corr$Correlation_P,
                        P_mirabilis = P.mirabilis_corr$Correlation_coefficient,
                        P_mirabilis_p = P.mirabilis_corr$Correlation_P,
                        S_aureus = S.aureus_corr$Correlation_coefficient,
                        S_aureus_p = S.aureus_corr$Correlation_P)
# Filtering the feature with the correlation >= 0.5
best_feat <- best_feat[best_feat$E_coli >= 0.5 |
                         best_feat$K_pneumonia >= 0.5 |
                         best_feat$P_mirabilis >= 0.5 |
                         best_feat$S_aureus >= 0.5, ]
# Adding the metabolite name to the correlation table
best_feat <- left_join(best_feat, feat_corr_table)
# Extracting data of no identified metabolites
best_feat <- best_feat[!complete.cases(best_feat$Metabolite),]
# Replace the column name and ionization type "RTX5MS_EI" with unknown
best_feat$Feature_ID <- sub('RTX5MS_EI_','Unknown-', best_feat$Feature_ID)
best_feat$Feature_ID <- str_replace_all(best_feat$Feature_ID, '_', '.')

```

Create a matrix for the heatplot of Pearson's correlation.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Correlation table for the heatmap
hm_corr_table1 <- data.frame(Feature_ID = best_feat$Feature_ID,
                             E_coli = best_feat$E_coli,
                             K_pneumonia = best_feat$K_pneumonia,
                             P_mirabilis = best_feat$P_mirabilis,
                             S_aureus = best_feat$S_aureus)
# Adding row name
rownames(hm_corr_table1) <- hm_corr_table1$Feature_ID
# Delete extra information
hm_corr_table1 <- subset(hm_corr_table1, select = -c(Feature_ID))
# Converting DataFrame to data matrix
hm_corr_table1 <- data.matrix(hm_corr_table1, rownames.force = NA)
# Adding column name
colnames(hm_corr_table1) <- c("E. coli", "K. pneumonia", "P. mirabilis", "S. aureus")

```

Create a matrix showing the statistical significance of the correlation result.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}

# Correlation table for the heatmap
hm_sign_table1 <- data.frame(Feature_ID = best_feat$Feature_ID,
                             E_coli = best_feat$E_coli_p,
                             K_pneumonia = best_feat$K_pneumonia_p,
                             P_mirabilis = best_feat$P_mirabilis_p,
                             S_aureus = best_feat$S_aureus_p)
# Adding row name
rownames(hm_sign_table1) <- hm_sign_table1$Feature_ID
# Delete extra information
hm_sign_table1 <- subset(hm_sign_table1, select = -c(Feature_ID))
# Converting DataFrame to data matrix
hm_sign_table1 <- data.matrix(hm_sign_table1, rownames.force = NA)
# Adding column name
colnames(hm_sign_table1) <- c("E. coli", "K. pneumonia", "P. mirabilis", "S. aureus")

```

Plot the correlation heatplot.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, fig.width = 5, fig.height = 6}

# Color scale legend
lgd1a <- Legend(col_fun = col_fun,
                title = "Correlation coefficient",
                direction = "horizontal" )
# Packed legends
pd = packLegend(lgd1a, direction = "horizontal")
# Heatmap plot
#png(file="Result/notame_Result/HS_GCMS/FigureA1.png", width = 5, height = 6, units = "in", res= 300)
hm_corr1 <- Heatmap(hm_corr_table1, col = col_fun, cluster_rows = FALSE,
                   cluster_columns = FALSE, column_names_gp = gpar(fontface = "italic"),
                   border_gp = grid::gpar(col = "black", lty = 0.02),
                   rect_gp = grid::gpar(col = "black", lwd = 0.75),
                   top_annotation = ann_corr,
                   cell_fun = function(j, i, x, y, w, h, fill) {
                     if(hm_sign_table1[i, j] < 0.05) {
                       grid.text(sprintf("%.1f", hm_corr_table1[i, j]), x, y,
                                 gp = gpar(fontsize = 10, fontface = "bold"))
                       } else if(hm_sign_table1[i, j] > 0.05) {
                         grid.text(sprintf("%.1f", hm_corr_table1[i, j]), x, y,
                                   gp = gpar(fontsize = 10))}},
                   show_heatmap_legend = F)
hm_corr1
draw(pd, x = unit(7.5, "cm"), y = unit(1, "cm"), just = c("left", "bottom"))
#dev.off()

```

# UpSet plot

The UpSet plot was implemented to inspect the unique or shared features of the honeys from each bee species and collection site (beehive). 

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Mean of technical replicate
upset_aver <- summary_statistics(compressed, grouping_cols = "Group")
# Dataframe with average values by the collection sites (beehive)
upset_mat <- data.frame(Feature_ID = upset_aver$Feature_ID,
                        ata001 = upset_aver$ATA001_mean,
                        er001 = upset_aver$ER001_mean,
                        fied001 = upset_aver$FIED001_mean,
                        rgb004 = upset_aver$RGB004_mean,
                        rgy001 = upset_aver$RGY001_mean,
                        rig003 = upset_aver$RIG003_mean,
                        rig005 = upset_aver$RIG005_mean,
                        rymg001 = upset_aver$RYMG001_mean,
                        ryta006 = upset_aver$RYTA006_mean)
# Adding rownames
row.names(upset_mat) <- upset_mat$Feature_ID
upset_mat$Feature_ID <- NULL
# Adding colnames
colnames(upset_mat) <- c("T. angustula ATA001",
                         "T. angustula ER001",
                         "M. fuscopilosa FIED001",
                         "M. fasciculata RGB004",
                         "M. fuscopilosa RGY001",
                         "M. fuscopilosa RIG003",
                         "M. fasciculata RIG005",
                         "M. fasciculata RYMG001",
                         "T. angustula RYTA006")
# Change the values by presence using one (1) or absence using 0
upset_mat[upset_mat > 0] <- 1       # presence 
upset_mat[is.na(upset_mat)] <- 0    # absence
# Make the combination matrix
comb_mat = make_comb_mat(upset_mat)
#png(file="Result/notame_Result/HS_GCMS/FigureA2_1.png", width = 7, height = 3.5, units = "in", res= 300)
upset_plot <- UpSet(comb_mat,
                    row_names_gp = gpar(fontsize = 10, fontface = "italic"),
                    top_annotation =
                      upset_top_annotation(comb_mat,
                                           gp = gpar(fill = "#F8766D",
                                                     col = "#F8766D"),
                                           add_numbers = TRUE,
                                           annotation_name_rot = 90,),
                    right_annotation =
                      upset_right_annotation(comb_mat,
                                             gp = gpar(fill = "#1C6AA8",
                                                     col = "#1C6AA8"),
                                             add_numbers = TRUE),
                    set_order = c("T. angustula ATA001",
                                  "T. angustula ER001",
                                  "T. angustula RYTA006",
                                  "M. fuscopilosa FIED001",
                                  "M. fuscopilosa RGY001",
                                  "M. fuscopilosa RIG003",
                                  "M. fasciculata RGB004",
                                  "M. fasciculata RIG005",
                                  "M. fasciculata RYMG001"))
#pdf(file="Result/notame_Result/HS_GCMS/FigureA2.pdf", width = 7, height = 3.5)
upset_plot
#dev.off()

```

Finish a record.

```{r}

finish_log()

```
