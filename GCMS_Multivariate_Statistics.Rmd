...
peak_height <- mode@assays@data@listData[[1]]
# Extracting phenotypic data
pheno_data <- data.frame(mode@colData@listData)
...
# PCA plot
raw_pca <- ggplot(scores,
       aes(PC1, PC2, shape = Group, color = Group)) +
  geom_point(size = 3) +
  guides(x=guide_axis(title = "PC1 (25.76 %)"),
         y=guide_axis(title = "PC2 (18.16 %)")) +
  labs(shape = 'Group', color= 'Group') +
  theme_classic() +
  theme(legend.text = element_text(face="italic")) +
  theme(legend.position = c(0.78, 0.79),
...
merged <- merge_notame_sets(corrected_r, corrected_b, corrected_l, qc_sample, merge = 'samples')

merge_notame_sets(corrected_r, corrected_l, corrected_b,  merge = 'samples')


merged <- merge_notame_sets(corrected_r, corrected_b, merge = 'samples') ##
merged <- merge_notame_sets(corrected_b, corrected_l, merge = 'samples')
merged <- merge_notame_sets(corrected_l, qc_sample, merge = 'samples')
merged2 <- merge_notame_sets(merged, corrected_l, merge = 'samples')
