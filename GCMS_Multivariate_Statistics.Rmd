---
title: "Chemical characterization of the metabolites present in different structures of B. grandiflora by GC-MS metabolomics - Multivariate statistics"
...
This R Script aims to record the procedure for the statistical analysis of the metabolomic profile of different parts (root, bark y leaf) of the *B. grandiflora* plant.      
...
```{r echo=TRUE, message=FALSE}
 ...
 #Notame package installationn
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")

BiocManager::install("notame")

#NotameViz package installation
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
...
Then, the libraries were loaded.
...
data <- import_from_excel(file = "Data/notame/GCMS_B_grandiflora_feat_list_2notame.xlsx",
                        sheet = 4, corner_row = 5, corner_column = "M",
...
Then, features with low detection rates are flagged and removed.

The “*notame*” package uses two criteria to flag these features: the feature's presence in a percentage of QC injections and the feature's presence in a percentage within a sample group or class. Also, we improve the data acquisition using QCs pooled from each experimental group of samples separately insted of QCs pooled from all samples; we considered features as "Low_qc_detection" if the feature is in at least of 60 % of QCs.

```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}

# Low detection rate for the all dataset
low_dr <- flag_detection(mode, group = 'Group', qc_limit = 0, group_limit = 3/5)
# Extract clean data
low_dr <- drop_flagged(low_dr)
# Adding the "Low_qc_detection" flag
low_detect1 <-  data.frame(low_dr@elementMetadata@listData)
low_detect1 <- low_detect1 %>% 
   mutate(Flag = case_when(`Detection_rate_Group_Leaves.pool` < 0.6 &
                            `Detection_rate_Group_Roots.pool` < 0.6 &
                            `Detection_rate_Group_Stem.bark.pool` < 0.6 &
                            `Detection_rate_QC` < 0.6 ~ "Low_qc_detection"))


low_dr@elementMetadata@listData <- low_detect1
...
names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "Flag"] <- "Flag_all"

names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "RSD"] <- "RSD_all"

names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "D_ratio"] <- "D_ratio_all"

names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "RSD_r"] <- "RSD_r_all"

names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "D_ratio_r"] <- "D_ratio_r_all"

names(mode@elementMetadata@listData)[names(mode@elementMetadata@listData) == "DC_note"] <- "DC_note_all"

# Adding new empty Flag column
lista1 <- data.frame(mode@elementMetadata@listData) 
lista1$Flag <- NA
mode@elementMetadata@listData <- lista1
...
colData(roots) <- droplevels(colData(roots))
# Removal of Stem bark group from the dataset
roots <- roots[, roots$Plant_part != "Stem bark"]
colData(roots) <- droplevels(colData(roots))
# Using a pool of Roots samples as QC
levels(colData(roots)$Group)[levels(colData(roots)$Group)=="Roots pool"] <- "QC"
colData(roots)$QC <- colData(roots)$Group
levels(colData(roots)$QC)[levels(colData(roots)$QC)=="Roots"] <- "Sample"
...
colData(bark) <- droplevels(colData(bark))
# Removal of Roots group from the dataset
bark <- bark[, bark$Plant_part != "Roots"]
colData(bark) <- droplevels(colData(bark))
# Using a pool of Stem bark samples as QC
levels(colData(bark)$Group)[levels(colData(bark)$Group)=="Stem bark pool"] <- "QC"
colData(bark)$QC <- colData(bark)$Group
levels(colData(bark)$QC)[levels(colData(bark)$QC)=="Stem bark"] <- "Sample"
...
# Drop QC
leaves <- drop_qcs( mode)
# Removal of Stem bark group from the dataset
leaves <- leaves[, leaves$Plant_part != "Stem bark"]
colData(leaves) <- droplevels(colData(leaves))
# Removal of Roots group from the dataset
leaves <- leaves[, leaves$Plant_part != "Roots"]
colData(leaves) <- droplevels(colData(leaves))
# Using a pool of leaves samples as QC
levels(colData(leaves)$Group)[levels(colData(leaves)$Group)=="Leaves pool"] <- "QC"
colData(leaves)$QC <- colData(leaves)$Group
levels(colData(leaves)$QC)[levels(colData(leaves)$QC)=="Leaves"] <- "Sample"
...
# Removal of Stem bark group from the dataset
qc_sample <- qc_sample[, qc_sample$Plant_part != "Stem bark"]
colData(qc_sample) <- droplevels(colData(qc_sample))
# Removal of Roots group from the dataset
qc_sample <- qc_sample[, qc_sample$Plant_part != "Roots"]
colData(qc_sample) <- droplevels(colData(qc_sample))
# Removal of leaves group from the dataset
qc_sample <- qc_sample[, qc_sample$Plant_part != "leave"]
colData(qc_sample) <- droplevels(colData(qc_sample))

levels(colData(qc_sample)$Group)[levels(colData(qc_sample)$Group)=="qc_sample"] <- "QC"
colData(qc_sample)$QC <- colData(qc_sample)$Group
levels(colData(qc_sample)$QC)[levels(colData(qc_sample)$QC)=="qc_sample"] <- "Sample"
...
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "Flag"] <- "Flag_root"
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "RSD"] <- "RSD_root"
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "D_ratio"] <- "D_ratio_root"
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "RSD_r"] <- "RSD_r_root"
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "D_ratio_r"] <- "D_ratio_r_root"
names(corrected_r@elementMetadata@listData)[names(corrected_r@elementMetadata@listData) == "DC_note"] <- "DC_note_root"
# Adding new empty Flag column

lista2 <- data.frame(corrected_r@elementMetadata@listData) 
lista2$Flag_r <- NA
corrected_r@elementMetadata@listData <- lista2

# Adding a unique identifier for the drift result of the stem bark dataset
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "Flag"] <- "Flag_bark"
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "RSD"] <- "RSD_bark"
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "D_ratio"] <- "D_ratio_bark"
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "RSD_r"] <- "RSD_r_bark"
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "D_ratio_r"] <- "D_ratio_r_bark"
names(corrected_b@elementMetadata@listData)[names(corrected_b@elementMetadata@listData) == "DC_note"] <- "DC_note_bark"
# Adding new empty Flag column
lista3 <- data.frame(corrected_b@elementMetadata@listData) 
lista3$Flag_b <- NA
corrected_b@elementMetadata@listData <- lista3

# Adding a unique identifier for the drift result of the leaves dataset
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "Flag"] <- "Flag_leave"
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "RSD"] <- "RSD_leave"
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "D_ratio"] <- "D_ratio_leave"
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "RSD_r"] <- "RSD_r_leave"
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "D_ratio_r"] <- "D_ratio_r_leave"
names(corrected_l@elementMetadata@listData)[names(corrected_l@elementMetadata@listData) == "DC_note"] <- "DC_note_leave"
# Adding new empty Flag column
lista4 <- data.frame(corrected_l@elementMetadata@listData) 
lista4$Flag_l <- NA
corrected_l@elementMetadata@listData <- lista4
```
...
# Verify and sort the rownames
if (!identical(rownames(corrected_r), rownames(corrected_l)) || 
    !identical(rownames(corrected_r), rownames(corrected_b))) {
  
  common_features <- Reduce(intersect, list(
    rownames(corrected_r), 
    rownames(corrected_l), 
    rownames(corrected_b)
  ))
  
  corrected_r <- corrected_r[common_features, ]
  corrected_l <- corrected_l[common_features, ]
  corrected_b <- corrected_b[common_features, ]
}
...
# Combine fData while keeping all columns unique
combined_fData <- cbind(
  rowData(corrected_r),
  rowData(corrected_l),
  rowData(corrected_b),
  rowData(qc_sample)
...
# Or if there are duplicate columns, select only the unique ones
all_rowdata <- list(
  as.data.frame(rowData(corrected_r)),
  as.data.frame(rowData(corrected_l)), 
  as.data.frame(rowData(corrected_b)),
  as.data.frame(rowData(qc_sample))
)


test_r <- data.frame(rowData(corrected_r))

# Find unique column names
unique_columns <- unique(unlist(lapply(all_rowdata, names)))

# Create combined rowData
combined_fData <- DataFrame(
  lapply(unique_columns, function(col) {
    # Search for the column in each dataset, use the first one that has it.
    for(i in 1:4) {
      if(col %in% names(all_rowdata[[i]])) {
        return(all_rowdata[[i]][[col]])
      }
    }
    return(rep(NA, nrow(corrected_r)))  # If it does not exist in either, fill with NA.
  })
)
names(combined_fData) <- unique_columns

# Create new SummarizedExperiment
merged <- SummarizedExperiment(
  assays = list(assay = combined_assay),
  colData = combined_pData,
  rowData = combined_fData
  )
...
summary(rowData(merged)$mass)  
summary(rowData(merged)$RT)    
...
InChI_Keys <- c('2-Heptanone' = "DTOSIQBPPRVQHS-PDBXOOCHSA-N")
# Get classification
